# Back end
scoreboard objectives add time dummy
scoreboard players set milisecond time 0
scoreboard players set second time 0
scoreboard players set minute time 0
scoreboard players set tick time 0

scoreboard players set start time 0
scoreboard players set mizero time 0
scoreboard players set sezero time 0

# Data
scoreboard objectives add Milisecond dummy
scoreboard objectives add Second dummy
scoreboard objectives add Minute dummy

# To wipe the data
scoreboard players set wipe time 0

# Position
scoreboard players set z time 0
scoreboard players set x time 0

# Ensuring tick is off
scoreboard players set time tick 0