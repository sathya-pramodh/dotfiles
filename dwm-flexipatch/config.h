...
static const char *altbarcmd =
    "$HOME/.config/polybar/launch.sh --hack"; /* Alternate bar launch command */
...
static Key keys[] = {
...
#if FLOATPOS_PATCH
    /* Note that due to key limitations the below example kybindings are
     * defined with a Mod3Mask, which is not always readily available. Refer to
     * the patch wiki for more details. */
    ...
    /* Client is positioned in a floating grid, movement is relative to
