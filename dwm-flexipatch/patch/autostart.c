void runautostart(void) {
  ...
  if ((xdgdatahome = getenv("XDG_DATA_HOME")) != NULL) {
    ...
    /* space for path segments, separators and nul */
    if ((pathpfx = malloc(strlen(home) + strlen(localshare) + strlen(dwmdir) +
                          3)) == NULL)
      return;
  ...
  if ((path = malloc(strlen(pathpfx) + strlen(autostartblocksh) + 2)) == NULL) {
    free(pathpfx);
    return;
  } else if (sprintf(path, "%s/%s", pathpfx, autostartblocksh) <= 0) {
    free(path);
    free(pathpfx);
  }
  ...
  if ((path = realloc(path, strlen(pathpfx) + strlen(autostartsh) + 4)) ==
    ...
    free(path);
    return;
  } else if (sprintf(path, "%s/%s", pathpfx, autostartsh) <= 0) {
    free(path);
    free(pathpfx);
  }
